...
/**
 * BaseAchRequestModel Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class BaseAchRequestModel implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'base_ach_request_model';
     ...
     * Sets bank_name
     *
     * @param string|null $bank_name Name of the bank holding the account.
     *
     * @return self
     */
    ...
    public function setNameOnAccount($name_on_account)
    {
        if ((mb_strlen($name_on_account) > 50)) {
            throw new \InvalidArgumentException('invalid length for $name_on_account when calling BaseAchRequestModel., must be smaller than or equal to 50.');
        }
        if ((mb_strlen($name_on_account) < 1)) {
            throw new \InvalidArgumentException('invalid length for $name_on_account when calling BaseAchRequestModel., must be bigger than or equal to 1.');
        }
    ...
    public function setToken($token)
    {
        if (!is_null($token) && (mb_strlen($token) > 36)) {
            throw new \InvalidArgumentException('invalid length for $token when calling BaseAchRequestModel., must be smaller than or equal to 36.');
        }
        if (!is_null($token) && (mb_strlen($token) < 1)) {
            throw new \InvalidArgumentException('invalid length for $token when calling BaseAchRequestModel., must be bigger than or equal to 1.');
        }
