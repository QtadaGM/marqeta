...
/**
 * AccountConfigReq Class Doc Comment
 *
 * @category Class
 ...
 */
class AccountConfigReq implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AccountConfigReq';
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['billing_cycle_day']) && ($this->container['billing_cycle_day'] > 28)) {
            $invalidProperties[] = "invalid value for 'billing_cycle_day', must be smaller than or equal to 28.";
        }

        if (!is_null($this->container['billing_cycle_day']) && ($this->container['billing_cycle_day'] < 1)) {
     ...
     *
     * @param int|null $billing_cycle_day Day of month the billing cycle starts.  If an override value is not provided, the default value is derived from the bundle.
     *
     * @return self
     */
    public function setBillingCycleDay($billing_cycle_day)
    {
        ...
        if (!is_null($billing_cycle_day) && ($billing_cycle_day > 28)) {
            throw new \InvalidArgumentException('invalid value for $billing_cycle_day when calling AccountConfigReq., must be smaller than or equal to 28.');
        }
        if (!is_null($billing_cycle_day) && ($billing_cycle_day < 1)) {
            throw new \InvalidArgumentException('invalid value for $billing_cycle_day when calling AccountConfigReq., must be bigger than or equal to 1.');
        }
     ...
     * Sets card_level
     *
     * @param string|null $card_level Level of the credit card.
     *
     * @return self
     */
    ...
    {
        $allowedValues = $this->getCardLevelAllowableValues();
        if (!is_null($card_level) && !in_array($card_level, $allowedValues, true)) {
     ...
     *
     * @param int|null $payment_due_day Day of month the payment for the previous billing cycle is due.  This field is deprecated. Use the `account.payment_due_interval` field instead. To retrieve `payment_due_interval`, see <</core-api/policies#retrieveProductPolicy, Retrieve credit product policy, payments.payment_due_interval>>.
     *
     * @return self
     * @deprecated
     */
    public function setPaymentDueDay($payment_due_day)
    {
        ...
        if (!is_null($payment_due_day) && ($payment_due_day > 31)) {
            throw new \InvalidArgumentException('invalid value for $payment_due_day when calling AccountConfigReq., must be smaller than or equal to 31.');
        }
        if (!is_null($payment_due_day) && ($payment_due_day < 31)) {
            throw new \InvalidArgumentException('invalid value for $payment_due_day when calling AccountConfigReq., must be bigger than or equal to 31.');
        }
