...
/**
 * PaymentScheduleCreateReq Class Doc Comment
 *
 * @category Class
 ...
 */
class PaymentScheduleCreateReq implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'PaymentScheduleCreateReq';
    ...
    public function listInvalidProperties()
    {
        ...
        if ((mb_strlen($this->container['payment_source_token']) > 36)) {
     ...
     * Sets amount
     *
     * @param float|null $amount Amount of the payment.  Required if `amount_category` is `FIXED`.
     *
     * @return self
     */
    ...
    public function setDescription($description)
    {
        if (!is_null($description) && (mb_strlen($description) > 255)) {
            throw new \InvalidArgumentException('invalid length for $description when calling PaymentScheduleCreateReq., must be smaller than or equal to 255.');
        }
     ...
     *
     * @param \DateTime|null $next_payment_impact_date Date to make a one-time payment.  Required if frequency is `ONCE`.
     *
     * @return self
     */
     ...
     * Sets payment_day
     *
     * @param string|null $payment_day Day on which monthly payments are made.  Required if `frequency` is `MONTHLY`.
     *
     * @return self
     */
     ...
     *
     * @param string $payment_source_token Unique identifier of the payment source.
     *
     * @return self
     */
    public function setPaymentSourceToken($payment_source_token)
    {
        if ((mb_strlen($payment_source_token) > 36)) {
            throw new \InvalidArgumentException('invalid length for $payment_source_token when calling PaymentScheduleCreateReq., must be smaller than or equal to 36.');
        }
     ...
     * Sets token
     *
     * @param string|null $token Unique identifier of the payment schedule.
     *
     * @return self
     */
    public function setToken($token)
    {
        if (!is_null($token) && (mb_strlen($token) > 36)) {
            throw new \InvalidArgumentException('invalid length for $token when calling PaymentScheduleCreateReq., must be smaller than or equal to 36.');
        }
        if (!is_null($token) && (!preg_match("/(?!^ +$)^.+$/", $token))) {
            throw new \InvalidArgumentException("invalid value for $token when calling PaymentScheduleCreateReq., must conform to the pattern /(?!^ +$)^.+$/.");
        }
