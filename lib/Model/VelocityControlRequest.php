...
/**
 * VelocityControlRequest Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class VelocityControlRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'velocity_control_request';
    ...
    public function setAmountLimit($amount_limit)
    {
        ...
        if (($amount_limit < 0)) {
            throw new \InvalidArgumentException('invalid value for $amount_limit when calling VelocityControlRequest., must be bigger than or equal to 0.');
        }
     ...
     *
     * @param bool|null $include_credits If set to `true`, original credit transactions (OCT) are subject to control. Your request can contain either a `money_in_transaction` object or the `include_credits` field, not both.
     *
     * @return self
     */
    ...
    public function setName($name)
    {
        if (!is_null($name) && (mb_strlen($name) > 255)) {
            throw new \InvalidArgumentException('invalid length for $name when calling VelocityControlRequest., must be smaller than or equal to 255.');
        }
        if (!is_null($name) && (mb_strlen($name) < 0)) {
            throw new \InvalidArgumentException('invalid length for $name when calling VelocityControlRequest., must be bigger than or equal to 0.');
        }
    ...
    public function setToken($token)
    {
        if (!is_null($token) && (mb_strlen($token) > 36)) {
            throw new \InvalidArgumentException('invalid length for $token when calling VelocityControlRequest., must be smaller than or equal to 36.');
        }
        if (!is_null($token) && (mb_strlen($token) < 1)) {
            throw new \InvalidArgumentException('invalid length for $token when calling VelocityControlRequest., must be bigger than or equal to 1.');
        }
     ...
     * Sets usage_limit
     *
     * @param int|null $usage_limit Maximum number of times a card can be used within the time period defined by the `velocity_window` field.  If `velocity_window` is set to `TRANSACTION`, do not include a `usage_limit` in your request.  Set to `-1` to indicate that the card can be used an unlimited number of times.
     *
     * @return self
     */
    public function setUsageLimit($usage_limit)
    {
        ...
        if (!is_null($usage_limit) && ($usage_limit < -1)) {
            throw new \InvalidArgumentException('invalid value for $usage_limit when calling VelocityControlRequest., must be bigger than or equal to -1.');
        }
